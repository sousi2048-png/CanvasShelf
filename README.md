...
Use **Refresh folders** on the home page to scan `../Arts/` and register newly added image folders without restarting the server. Each collection remembers its own sort order (newest, oldest, or name) in the browser and local server. Name sorting flows left-to-right in rows.
...
Add an entry to `gallery_collections.json`, or place a new image folder under `../Arts/` and run:
